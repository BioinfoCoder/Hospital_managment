...
select d.Diagnosis , p.name ,p.Gender 
from [dbo].[PATIENT_DIAGNOSIS] d
join [dbo].[Patient] p
on d.Patient_ID = p.patient_id
where p.Gender = 'Male'
group by d.Diagnosis ,  p.name , p.Gender;


select d.Diagnosis , p.name ,p.Gender g
from [dbo].[PATIENT_DIAGNOSIS] d
join [dbo].[Patient] p
on d.Patient_ID = p.patient_id
...
join [dbo].[Patient] p
on d.Patient_ID = p.patient_id
where p.Gender = 'Female' and p.primary_check > 5

select count(*)
from [dbo].[PATIENT_DIAGNOSIS] d
join [dbo].[Patient] p
on d.Patient_ID = p.patient_id
where p.Gender = 'Male' and p.primary_check > 5
...
select TOP 5 d.head , d.dept_name
from [dbo].[department] d
group by d.dept_name ,  d.head
order by d.head desc

SELECT *
from [dbo].[PATIENT_DIAGNOSIS] d
join [dbo].[Patient] p
on d.Patient_ID = p.patient_id
where d.Diagnosis = 'Hypertension' and p.Gender = 'Male';

SELECT *
from [dbo].[PATIENT_DIAGNOSIS] d
join [dbo].[Patient] p
on d.Patient_ID = p.patient_id
where d.Diagnosis = 'Hypertension' and p.Gender = 'Female';

select count(*) c ,n.position
from [dbo].[Nurse] n 
where n.registered = 'Yes'
group by n.position
order by c desc

select count(*) c, n.position
from [dbo].[Nurse] n 
where n.registered = 'No'
group by n.position
order by c

select COUNT(d.Diagnosis) number_of_diagnostic_cases, d.Prescription
from [dbo].[PATIENT_DIAGNOSIS] d
group by d.Prescription 
having  COUNT(d.Diagnosis) > 1 



with t1 as (select d.Diagnosis
from [dbo].[PATIENT_DIAGNOSIS] d
join [dbo].[Patient] p
on d.Patient_ID = p.patient_id
where p.Gender = 'Male' and p.primary_check > 5
